...
OUT_HTML = Path("reports/fashion_france_fashion_week_contextual_analysis_2026_v3.html")
OUT_CSV = Path("data/fashion_france_fashion_week_contextual_signals_2026_v3.csv")
...
known_signal_total = float(known_signal_perf["impressions"].sum())
known_signal_cumulative = (
    known_signal_perf["impressions"].cumsum() / known_signal_total * 100
    if known_signal_total
    else pd.Series(dtype="float64")
)
main_category_count = int((known_signal_cumulative < 90).sum() + 1) if known_signal_total else 0
main_category_share = (
    float(known_signal_perf.head(main_category_count)["impressions"].sum() / known_signal_total * 100)
    if known_signal_total and main_category_count
    else 0
)
...
context_rows = [
    {"name": row[TOPIC_COL], "imps": int(row["impressions"])}
    for _, row in topic_totals.head(8).iterrows()
]
topic_pills = [
    {
        "name": row[TOPIC_COL],
...
<div class="kpi-strip">
  ...
  <div class="kpi"><div class="kpi-value"><em>{main_category_count}</em></div><div class="kpi-label">Main Categories</div><div class="kpi-sub">{main_category_share:.1f}% of known-category impressions</div></div>
...
<div class="grid-section">
  <div class="grid-2">
    ...
    <div class="panel"><div class="section-label">Top topic clusters · impressions</div><div id="contextBars"></div></div>
  </div>
</div>
...
<div class="grid-section">
  <div class="grid-2">
    <div class="panel" style="border: 0.5px solid var(--border-primary);">
      <div class="section-label">Topic cluster scale · top 8</div>
      <div style="position:relative;height:240px;"><canvas id="contextChart" role="img" aria-label="Fashion France topic cluster scale"></canvas></div>
    </div>
    <div class="panel" style="border: 0.5px solid var(--border-primary);">
